...
function loadSession(){
  // Firebase onAuthStateChanged asıl giriş yöneticisi
  // localStorage sadece hızlı UI önizleme için (spinner süresince)
  try{
    const raw=localStorage.getItem('bk_user_v4');
    if(raw){
      const u=JSON.parse(raw);
      if(u&&u.isLoggedIn){
        // Cache varsa arka planda UI'yı hazırla ama enterApp çağırma
        // Firebase profil gelince onUserReady → enterApp çalışacak
